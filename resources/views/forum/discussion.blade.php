...
<div id="chatter" class="discussion margin-top">

	<div id="chatter_header" style="background-colors:{{ $discussion->color }}">
		<div class="container">
			<a class="back_btn" href="/{{ Config::get('chatter.routes.home') }}"><i class="chatter-back"></i></a>
			<h3>{{ $discussion->title }}</h3><span class="chatter_head_details">Posted In {{ Config::get('chatter.titles.category') }}<a class="chatter_cat" href="/{{ Config::get('forum.routes.home') }}/{{ Config::get('forum.routes.category') }}/{{ $discussion->category->slug }}" style="background-color:{{ $discussion->category->color }}">{{ $discussion->category->name }}</a></span>
		
		</div>
	</div>

	@if(Session::has('alert'))
		<div class="chatter-alert alert alert-{{ Session::get('alert_type') }}">
			<div class="container">
	        	<strong><i class="chatter-alert-{{ Session::get('alert_type') }}"></i> {{ Config::get('forum.alert_messages.' . Session::get('alert_type')) }}</strong>
	        	{{ Session::get('alert') }}
				...
				<div class="conversation panel">
	                <ul class="discussions no-bg" style="display:block;">
	                	@foreach($posts as $post)
                                <li data-id="{{ $post->id }}" class="panel-body" data-markdown="{{ $post->markdown }}">
	            ...
	            @if(!Auth::guest())
	            	...
	            	<div id="new_response">
	            		...
	            		<div class="chatter_avatar">
		        			...
		        			@else
		        				<span class="chatter_avatar_circle" style="background-color:#<?= \App\Helpers\DataHelper::stringToColorCode(Auth::user()->email) ?>">
